...
public interface Product_BillRepo extends JpaRepository<Product_Bill, Long> {
	...
	@Query(value = "SELECT pb.createdDate, COUNT(pb.id) FROM Product_Bill pb "
			+ "WHERE (pb.isdelete = 'N') AND (pb.createdDate BETWEEN :fromDate AND :toDate) AND (pb.bill.typeStatus != 1 AND pb.bill.typeStatus != 2)"
			+ "GROUP BY pb.createdDate", nativeQuery = false)
	...
	@Query(value = "SELECT pb.product.name, SUM(pb.quantity) FROM Product_Bill pb "
			+ "WHERE pb.createdDate = DATE(NOW())"
			+ "GROUP BY pb.product.name, pb.createdDate", nativeQuery = false)
