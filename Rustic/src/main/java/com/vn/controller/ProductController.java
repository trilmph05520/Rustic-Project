...
//    private String SUB_IMG_SOURCE_WEB = "/sub-img/";
	...
	@PreAuthorize("hasAnyAuthority('Administrators','Staffs')")
	public @ResponseBody String addProduct(@RequestBody(required = false) ProductModel model) {
	...
	@PreAuthorize("hasAnyAuthority('Administrators','Staffs')")
	public @ResponseBody String updatePost(@PathVariable("id") Long id,
			@RequestBody(required = false) ProductModel model) {
		Gson gson = new GsonBuilder().setPrettyPrinting().create();
		Product product = productService.findOne(id);
		Map<String, Object> responseMap = new HashMap<>();
		String filePathMain = ROOT_FOLDER + MAIN_ADDRESS;
