...
public class PaymentController {
	...
	public @ResponseBody String paymentLive(@RequestBody(required = false) BillModel billModel, HttpSession session,
...
//                        bill.setAuthUser(user);
					...
					for (Map.Entry<Long, Cart> eachValid : map.entrySet()) {
						...
						if (pro.getQuantity() < eachValid.getValue().getQuantity()) {
							responeMap.put("limit", "Đặt hàng không thành công! Số lượng hàng "
									+ eachValid.getValue().getProduct().getName() + " trong kho không đủ ( chỉ còn "
									+ eachValid.getValue().getProduct().getQuantity() + " sản phẩm )");
							checkPro = false;
							break;
						}
					}
					...
					new Thread(() -> {
						try {
							...
							final String htmlContent = ThymeleafUtil.getHtmlContentInClassPath(
									"html/MailCustomerOrderProduct.html", (HashMap<String, Object>) responseMapMail);
							mailSender.sendSimpleMailWarningTLS("Rustic<trilmph05520@fpt.edu.vn>", billModel.getEmail(),
									"[Rustic] Email đơn đặt hàng Quý Khách", htmlContent);
						} catch (Exception e) {
							e.printStackTrace();
						}
					}).start();
	...
	public @ResponseBody String payingByVNPay(@RequestBody(required = false) BillModel model, HttpServletRequest req,
			HttpSession session) {
		Gson gson = new GsonBuilder().setPrettyPrinting().create();
		try {
				...
				if (isSuccess) {
					new Thread(() -> {
						try {
							...
							final String htmlContent = ThymeleafUtil.getHtmlContentInClassPath(
									"html/MailCustomerOrderProduct.html", (HashMap<String, Object>) responseMapMail);
							mailSender.sendSimpleMailWarningTLS("Rustic<trilmph05520@fpt.edu.vn>", model.getEmail(),
									"[Rustic] Email đơn đặt hàng Quý Khách", htmlContent);
						} catch (Exception e) {
							e.printStackTrace();
						}
